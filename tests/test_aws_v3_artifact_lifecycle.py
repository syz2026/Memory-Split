...
def _advance_evidence(seed: int) -> dict[str, object]:
    return {
        ...
        "evaluation_receipt_sha256": "5" * 64,
        "evaluation_receipt_uri": (
            f"s3://memorysplit-prod/evaluations/seed-{seed}/receipts/"
            f"{'5' * 64}.json"
        ),
...
def test_mocked_v3_terminal_evaluate_collect_advance_lifecycle(
    ...
    class Store:
        ...
        def read_evaluation(self, manifest_sha256):
            assert manifest_sha256 == manifest.sha256
            return dict(evaluation_state)
    ...
    assert advanced["resources"]["evaluation_receipt_sha256"] == (
