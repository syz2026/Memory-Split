...
# The zone and the memory-headroom gate were frozen for this study before any
# protected outcome existed, so they are no longer pending. The repository URI
# was always bound; only the image digest behind it is outstanding.
#
# The six hardware and image fields were attested on the qualified node
# i-05de75630c4774cdd inside Capacity Block cr-010377794039df5cc: the GPU facts
# from nvidia-smi (corroborated independently by torch reporting sm_100 on all
# eight devices), the AMI facts from ec2 describe-images. What remains pending
# are exactly the three fields that can only be frozen from inside the
# qualified container runtime image, which is still being built.
...
def test_shipped_profile_binds_every_frozen_p6_b200_fact():
    ...
    assert set(shipped.pending_confirmation) == set(PENDING_FIELDS)
    assert shipped.availability_zone == "us-east-1d"
    assert shipped.memory_headroom_fraction_floor == 0.2
...
def test_readiness_report_marks_the_exact_profile_as_not_yet_admissible():
    ...
    assert sorted(report["pending_confirmation"]) == sorted(PENDING_FIELDS)
