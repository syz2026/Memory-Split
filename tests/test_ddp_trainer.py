...
def _missing_token_signal_request_worker(
    rank,
    world_size,
    port,
    cfg,
    ...
    os.environ.update(
        {
            "MASTER_ADDR": "127.0.0.1",
            "MASTER_PORT": str(port),
            "MS_CHECKPOINT_REQUEST_ARM": "dense",
            "MS_CHECKPOINT_REQUEST_TOKEN_FILE": request_token_path,
            "MS_RANK_ZERO_PID_FILE": pid_path,
            "RANK": str(rank),
            "WORLD_SIZE": str(world_size),
            "LOCAL_RANK": str(rank),
    ...
    try:
        trainer = Trainer(cfg)
        if rank == 0:
            os.chown(trainer.out_dir, -1, os.getegid())
        ...
        while not release.exists() and time.monotonic() < deadline:
            time.sleep(0.01)
        if not release.exists():
            raise RuntimeError("missing-token signal test was not released")
        serviced = trainer._service_checkpoint_request()
        Path(result_dir, f"serviced-{rank}").write_text(str(serviced))
    except BaseException:
        Path(result_dir, f"rank-{rank}.txt").write_text(traceback.format_exc())
...
def test_cpu_gloo_missing_signal_token_is_abandoned_by_all_ranks(tmp_path):
    token_path = tmp_path / "tokens.bin"
    (np.arange(80) % 32).astype(np.uint16).tofile(token_path)
    cfg = tiny_config(tmp_path / "missing-token-ddp", token_path)
    cfg["condition"] = "dense"
    request_token_path = (
        Path(cfg["out_dir"]) / "checkpoint-request.json"
    )
    world_size = 2
    port = _free_port()
    result_dir = tmp_path / f"missing-token-results-{port}"
    result_dir.mkdir()
    pid_path = tmp_path / "missing-token-rank-zero.pid"
    context = mp.get_context("spawn")
    processes = [
        context.Process(
            target=_missing_token_signal_request_worker,
            args=(
                rank,
                world_size,
                port,
    ...
    assert ready, "rank zero did not atomically publish its PID file"
    assert not alive, f"DDP missing-token workers deadlocked\n{errors}"
    assert [process.exitcode for process in processes] == [0, 0], errors
    assert [
        (result_dir / f"serviced-{rank}").read_text()
        for rank in range(world_size)
    ] == ["False", "False"]
    assert not request_token_path.exists()
    assert not (Path(cfg["out_dir"]) / "ckpt.pt").exists()
