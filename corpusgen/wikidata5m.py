...
def size_and_sha256(path: str | Path) -> tuple[int, str]:
    source = Path(path)
...
def safe_extract_archives(
    ...
    *,
    ...
    """Verify and atomically publish regular archive members.

    The destination must not already exist. All archives and member paths are
    validated before a private sibling directory is created. Only a completed
    extraction is renamed to the public destination.
    """
    ...
    _validate_archive_members(archives, destination)
    ...
    try:
        extracted = _extract_archives(archives, private)
    ...
    return tuple(
        destination / path.relative_to(private)
